...
export const ProductListComponent = ({ _id, imgSrc, title, type, price, inStock, fastDelivery, onSale, discount, qty }) => {
    
    const { state: cartState, dispatch } = useCart();
    const navigate = useNavigate();
    const [ wishlistButton, setButtonState ] = useState("favorite_border")

    const wishlistToggle = ({ _id, imgSrc, title, type, price, inStock, fastDelivery, onSale, discount, qty }) => {
        if(wishlistButton === "favorite_border") {
            setButtonState("favorite")
            return dispatch(addToWishlist({ _id, imgSrc, title, type, price, inStock, fastDelivery, onSale, discount, qty }))
        }
        if(wishlistButton === "favorite"){
            setButtonState("favorite_border")
            return dispatch(removeFromWishlist({ _id, imgSrc, title, type, price, inStock, fastDelivery, onSale, discount, qty }))
        }
    }
  return (
        <div key = {_id} className={`${styles.card1} card ecom`}>
            <div className="badge-h">
                <i className={`material-icons ${styles.materialIcons}`} onClick = {() => wishlistToggle({ _id, imgSrc, title, type, price, inStock, fastDelivery, onSale, discount, qty })} >{wishlistButton}</i>
